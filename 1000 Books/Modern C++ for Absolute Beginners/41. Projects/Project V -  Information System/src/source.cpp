...
namespace nm{
    ...
    Employee::Employee(const std::string& aname, int argage, const std::string& ajobtitle)
        : Person(aname, argage), jobtitle{ ajobtitle }
    {}

    Employee::Employee(const Employee& rhs)
        : Person(rhs), jobtitle{ rhs.jobtitle }
        {
        }
    ...
    Employee::Employee(Employee&& rhs) noexcept
        : Person(std::move(rhs)), jobtitle{ std::move(rhs.jobtitle) }
    {
    }
...
int main(){
    ...
    v2.emplace_back(std::make_unique<Employee>("Sample Name 4", 35, "Manager"));
    ...
    Employee o4("Sample name 7", 55, "Accountant");// copy assignment test:
    o3 = o4;
    o3.formatprint();
}

/*
 Please note that most of the time, we want a vector of objects, not a vector of
 pointers. But this is one of those rare situations where we indeed want a vector of
 pointers to maintain a collection of polymorphic objects.
 */
